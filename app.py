...
user_city = st.sidebar.selectbox(
    ...
    ["Bangalore", "Mumbai", "Delhi", "Chennimport os
import sqlite3
import streamlit as st
import google.generativeai as genai
from dotenv import load_dotenv

# --- Initialize Database (We will keep this here so your app doesn't crash, 
# but we can repurpose it later to save chat history!) ---
from db_setup import setup_database
setup_database()
...
if prompt := st.chat_input("Describe your symptoms, ask for a diet plan, or find a doctor..."):
    ...
    st.session_state.messages.append({"role": "assistant", "content": response.text})ai"]
)
